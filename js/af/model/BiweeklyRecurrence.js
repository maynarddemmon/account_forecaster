...
af.BiweeklyRecurrence = new JS.Class('BiweeklyRecurrence', af.Recurrence, {
    // Accessors ///////////////////////////////////////////////////////////////
    ...
    getItems: function(startDate, endDate, accum) {
        ...
        // Determine A vs B week set.
        const MILLIS_PER_WEEK = af.MILLIS_PER_WEEK,
            weekCount = Math.floor(time / MILLIS_PER_WEEK),
            isAWeek = weekCount % 2 === 0;
        if (isAWeek) {
            if (data.weekSet === 'b') time += MILLIS_PER_WEEK;
        } else {
            if (data.weekSet === 'a') time += MILLIS_PER_WEEK;
        }
        
        const label = self.label,
            amount = data.amount;
        while (time >= startTime && time <= endTime) {
            accum.push([self, time, label, amount]);
